...
tag:
  - Programming
description: 단순한 폼에서 시작해 useContext, Redux로 최적화해보자
---
...
여기서 문제는 객체의 한 속성만 바뀌게 되어도 참조값이 바뀌므로 전체 컴포넌트가 리렌더링 된다는 것입니다. 물론 속성의 수가 적을 때는 문제가 없지만, 속성의 수가 많아지면 리렌더링되는 컴포넌트의 수가 많아져 눈에 띄게 성능이 낮아지게 됩니다.

이번 글에서는 이런 문제를 해결하기 위해 기본적인 폼에서 시작해 useContext, Redux 순으로 최적화해보는 과정을 다루어 보려 합니다.

## 1. 기본적인 폼

<Basic />

객체(Object)를 상태 값으로 사용하는 가장 기본적인 폼을 구현해보았습니다. 폼에 직접 입력을 하시면 리렌더링이 어떻게 일어나는지 확인하실 수 있습니다. 리렌더링이 일어난 컴포넌트는 테두리의 색이 변하며, 횟수가 많아질수록 색이 더욱 진해집니다.
...
직접 테스트를 해보시면, 한 속성의 값만 바꾸더라도 전체 컴포넌트가 리렌더링 되는 것을 볼 수 있습니다. 왜 이런 일이 일어날까요? 사용자가 입력을 했을 때 일어나는 일을 순서대로 따라가보면 다음과 같습니다.
...
그러나 하나의 속성만을 수정했을 뿐인데 전체 컴포넌트가 리렌더링 되는 것은 낭비가 심합니다. 위의 예시에서는 속성이 4개 뿐이지만 속성의 수가 10개, 100개로 많아진다면 리렌더링으로 인해 성능이 현저히 떨어지게 될 것입니다. 변경이 일어난 속성만 리렌더링되도록 바꿀 수는 없을까요?
...
`<InfoProvider/>` 컴포넌트에서는 `info` 객체와 와 `setInfo` 함수를 선언한 뒤 Context API를 사용해 자식 컴포넌트에 전달합니다.
...
이렇게 Context API를 사용하면 리렌더링 대상을 획기적으로 줄일 수 있습니다. 지금부터는 조금 더 복잡한 문제를 풀어보겠습니다.
...
2. `name`의 값이 바뀌면 `copyName`의 값은 `name`의 값으로 바뀐다.
3. `copyName`의 값이 바뀌면 `doubleCopyName`의 값은 `copyName`의 값으로 바뀐다.

이러한 폼 컴포넌트는 어떻게 구현할 수 있을까요?

### 시도 1. useEffect 사용

일단 가장 빨리 시도해볼 수 있는 방법은 상태 값을 관리하는 컴포넌트인 `<InfoProvider/>` 컴포넌트에서 `useEffect`를 사용하는 방법입니다.
...
이 방법의 장점은 로직을 명확하게 할 수 있다는 점입니다. 첫 번째 `useEffect`를 보면 dependency 배열에 `info.name`이 있고, `setInfo` 함수 안에는 `copyName` 속성에 `info.name`을 대입하고 있습니다. 따라서 `name` 속성이 바뀌었을 때 `copyName` 속성이 그 값을 복사한다는 것을 명확하게 알 수 있습니다. 또한 모든 로직이 중앙에 모여있으므로 유지보수가 쉽다는 이점도 있습니다.
...
리렌더링을 총 한 번만 일으키기 위해서는 `setState`에 인자로 넘겨주는 콜백 함수 내에서 모든 로직을 수행하고 그 결과를 반환해야 합니다. 예를 들어 `<PropertyInput/>` 컴포넌트를 다음과 같이 수정할 수 있습니다.
...
모든 로직이 모여 있어 유지보수하기 쉽다는 장점은 여전하지만, 이 방법도 찜찜함이 남습니다. 컴포넌트의 재활용을 위해 `setValue`에 인자로 넘겨주는 콜백 함수를 분리하고 싶습니다. 이를테면 아래와 같이 `getNewInfoDependValue` 함수를 만들어 사용하는 것입니다.
...
그리고 이 함수를 사용하기 위해서는 부모 컴포넌트에서 Props를 통해 넘겨주어야 합니다.
...
export default function UseContext() {
  return (
    <InfoProvider>
      <h1 className="text-2xl font-bold">Depend (useEffect)</h1>
...
그러나 이런 코드는 작성하기도 힘들고, 유지보수 또한 어렵습니다. 위 함수를 포함한 모든 로직을 `<InfoProvider/>` 컴포넌트에서 관리하고 싶습니다. 물론 Context API를 사용해 `getNewInfoDependValue` 함수를 넘겨주는 것도 가능하지만, 상태 값도 아닌데 Context API를 사용하는 것은 비효율적입니다.

### 해결 방법. useReducer 사용

이 문제는 [useReducer](https://react.dev/reference/react/useReducer) 훅을 사용해 해결할 수 있습니다. 이 훅의 인수로는 크게 reducer 함수와 초기 상태 값 2가지가 있습니다. reducer 함수는 상태 값을 업데이트 하는 함수로, 위에서 정의한 `getNewInfoDependValue` 함수와 정확히 같은 역할을 합니다.
...
```tsx title="InfoProvider.tsx" {5, 8-12, 16, 18, 31-37}
...
그러면 아래와 같이 성능적으로도 최적화 되어있고, 유지보수도 쉬운 코드를 얻을 수 있습니다. 만약 서로 다른 타입의 폼을 만들더라도 리듀서 함수를 `InfoProvider` 컴포넌트에만 넘겨주면 되므로 재활용성도 높아졌습니다.
...
`<StoreProvider/>` 컴포넌트를 만들어 store를 전역적으로 사용할 수 있도록 합니다.
...
이제 위 컴포넌트들을 조합하여 `<WithRedux/>` 컴포넌트를 만들면 됩니다.

```tsx title="WithRedux.tsx"
"use client";
...
이렇게 구현된 컴포넌트는 부모 컴포넌트의 리렌더링도 일어나지 않고, 두 속성 사이의 의존성이 있는 경우에도 한 번만 리렌더링 됩니다.

> 해당 포스트의 코드들은 가독성을 높이기 위해 일부 코드가 수정되거나 생략되어 있습니다. 따라서 그대로 복사하여 사용할 경우 오류가 발생할 수 있습니다. 원본 코드는 [여기]()에서 확인하실 수 있습니다.
