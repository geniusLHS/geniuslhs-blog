...
tag:
  - Programming
description: Next.js, FastAPI, Supabase를 사용한 인공지능 챗봇의 구조와 작동 방식
---
...
- 백엔드 기술 스택: FastAPI + Supabase (deploy with AWS lightsail)
...
참고로 FastAPI에서의 응답은 NGINX를 거쳐 Next.js로 전달되는데, 이 때 NGINX에서도 오랜 시간동안 연결이 끊기지 않도록 따로 설정을 해주어야 합니다.
...
인공지능 챗봇을 실제로 구현해보며 전체적인 서비스의 구조 설계에 대해 더욱 깊게 이해할 수 있었습니다. 또한 이메일 인증이나 supabase의 cron, edge functions 등 다양한 기능들을 실제 프로덕트에 사용해볼 수 있도 좋았습니다.
